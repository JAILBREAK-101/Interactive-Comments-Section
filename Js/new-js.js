...
function create_functional_button(appending_parent) {
  ...
  let edit_btn_selection = document.querySelector('.edit').src = `./images/icon-edit.svg`;

  let delete_button = document.createElement('button');
  delete_button.innerHTML = "<img src='./images/icon-delete.svg'><img>";
  delete_button.classList.add('delete');
  let delete_btn_selection = document.querySelector('.delete').src = `./images/icon-delete.svg`;
...
let user_comment_container = document.querySelector('.default-comment .grid1')[3];
      ...
      // For typing the reply.
        ...
        btn_send.addEventListener('click', (e) => {
          ...
          function when_reply_changes () {
            ...
            var all_possible_replies = document.querySelectorAll('section.new-boxes');
            ...
            if  (type_area.value.indexOf(name) !== -1) {
              ...
              if (type_area.value.indexOf('@amyrobson') !== -1) {
                ...
                // function append_under_first(ind) {
                  ...
                  if (all_possible_replies.length >= 1 && type_area.value.indexOf('@amyrobson') !== -1) {
                    all_possible_replies[0].insertAdjacentElement('afterEnd',reply_box);
            ...
            // else if (index == 2) {
              ...
              function append_under_first(created_reply) {
                ...
                // FIRST CONDITION.
                if (created_reply.length > 1 && type_area.value.indexOf('@amyrobson') !== -1) {
                  append_created_reply(created_reply.length);
                }
                // SECOND CONDITION.
                else if (created_reply.length > 1 && type_area.value.indexOf('@daniel') !== -1) {
                  append_created_reply(created_reply.length);
                }
                // THIRD CONDITION.
                else if (created_reply.length > 1 && type_area.value.indexOf('@ramsesmiron') !== -1) {
                  append_created_reply(created_reply.length);
                }
              }
            }
